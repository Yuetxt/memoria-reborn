...
module.exports = {
    data: new SlashCommandSubcommandBuilder()
        .setName("servant_xp")
        .setDescription("Add xp to a servant")
    ...
    execute: async (interaction, _) => {
        ...
        const selector = new StringSelectMenuMultiplePagesBuilder()
            ...
            .addRealOptions(
                ...servants.map(servant => ({
                    ...
                    description: `Card ID : ${servant.id} | Lvl : ${servant.lvl} (${Math.round(servant.getXpProgress().progress * 100)}%) | Xp : ${servant.xp}`,
                }))
            )

        const row = new ActionRowBuilder().addComponents(selector)
        const container = new ContainerBuilder().addTextDisplayComponents((t) => t.setContent(`Select a servant to ${xp > 0 ? "add" : "remove"} **${Math.abs(xp)}** xp to !`)).addActionRowComponents(row)
        ...
        collector.on("end", async (i) => {
            if (servants.length !== 1) {
                ...
                const baseProgress = servant.getXpProgress()
                const baseLevel = servant.lvl
                servant.xp = Math.max(0, servant.xp + xp)
                servant.calculateLvl()
                await servant.save()
                interaction.deleteReply()
                interaction.followUp({
                    components: [
                        new ContainerBuilder().addSectionComponents(
                            new SectionBuilder().setThumbnailAccessory(t => t.setURL(servant.data.art)).addTextDisplayComponents(t => t.setContent(`### ${servant.data.name}`))
                                .addTextDisplayComponents(t => t.setContent(`**Owned by** : ${discordUser} (ID : ${servant.id})`))
                                .addTextDisplayComponents(t => t.setContent(`**Level **: ${baseLevel} (${Math.round(baseProgress.progress * 100)}%) -> **${servant.lvl} (${Math.round(servant.getXpProgress().progress * 100)}%)**`))
                        )
                    ],
